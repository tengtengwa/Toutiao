...
class EntNewsAdapter(private val entNews: ArrayList<EntNewsModel.EntNewsList>) :
    ...
    override fun getItemCount() = entNews.size

    override fun getItemViewType(position: Int): Int {
        if (position == 0 || position == 1) {
            return ITEM_STYLE_1
        } else if (entNews[position].time != null) {
            return if (entNews[position].tmp3pic != null) ITEM_STYLE_3 else ITEM_STYLE_2
        }
        return ITEM_STYLE_1
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val itemNewsInfo = entNews[position]
        when (holder) {
            is Item1Holder -> {
                holder.apply {
